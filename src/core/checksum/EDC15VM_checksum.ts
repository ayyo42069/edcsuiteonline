import { ChecksumResult } from "./EDC15P_checksum";

export class EDC15VM_checksum {
    ...
    public tdi41_checksum_search(file_buffer: Uint8Array, file_size: number): ChecksumResult {
        if (file_buffer.length === 0x40000) return this.tdi41_checksum_search_256kb(file_buffer, file_size);
        else if (file_buffer.length === 0x80000) return this.tdi41_checksum_search_512kb(file_buffer, file_size);
        else return this.tdi41_checksum_search_1024kb(file_buffer, file_size);
    }

    private tdi41_checksum_search_256kb(file_buffer: Uint8Array, file_size: number): ChecksumResult {
        let first_pass = true;
        let chk_oldvalue: number, chk_value: number, chk_start_addr: number, chk_end_addr: number;
        const chk_array = [0x000B80, 0x008000, 0x014000, 0x038B80, 0x040000];
    ...
    private tdi41_checksum_search_512kb(file_buffer: Uint8Array, file_size: number): ChecksumResult {
        let first_pass = true;
        let chk_oldvalue: number, chk_value: number, chk_start_addr: number, chk_end_addr: number;
    ...
    private tdi41_checksum_search_1024kb(file_buffer: Uint8Array, file_size: number): ChecksumResult {
        let first_pass = false;
        let chk_oldvalue: number, chk_value: number, chk_start_addr: number, chk_end_addr: number;
        const chk_array = [
            0x4000, 0x4B80, 0x10000, 0x14000, 0x14B80, 0x20000, 0x24000, 0x24B80, 0x30000, 0x34000,
            0x34B80, 0x40000, 0x44000, 0x44B80, 0x50000, 0x54000, 0x54B80, 0x60000, 0x64000, 0x64B80,
            0x70000, 0x74000, 0x74B80, 0x94000, 0xCC000, 0xD0000, 0xD0B80, 0xDC000, 0xE0000, 0xE0B80,
            0xEC000, 0xF0000, 0xF0B80, 0xFC000
        ];
        let seed_a = 0, seed_b = 0;
